...
Файл чека — це транспортний контейнер, що містить пакет пов’язаних облікових документів різних типів. Зазвичай до нього входять товарний документ і пов’язані з ним касові документи.

Наприклад, контейнер чека продажу може містити видаткову накладну та один або декілька прибуткових касових ордерів. Якщо оплату за чеком не здійснено, прибуткові касові ордери в контейнері відсутні.

Кожен контейнер може містити товари лише однієї торгової точки. Якщо чек, сформований на касі, включає товари з двох або більше торгових точок, для кожної з них створюється окремий контейнер. Кожен такий контейнер містить лише товари відповідної торгової точки та пов’язані з ними документи.

Контейнер також може містити виключно касові документи — наприклад, якщо його сформовано для чека внесення або вилучення коштів.
...
|Amount		            			| + |Double	|Сума за документом. Для товарного документу - це сума по всіх товарах без заокруглення фінальної суми. Для платіжного документу це сума конкретної форми оплати.|
|AmountPaid		        			| + |Double	|Сума оплати. Для товарного документу - це сума всіх платежів, якими чек закривається, без решти, але з урахуванням заокруглення готівки, якщо застосовується. Для платіжного документу - це та сама сума конкретної форми оплати.|
...
|CurrencyId		                    | + |Byte	|Валюта оплати. ID першої валюти дорівнює 1, другої - 2 і так далі|
|CurrencyRate		                | + |Double	|Курс валюти відносно базової. =1, якщо працюєте з однією валютою|
...
|FiscalRegisterId		            | - |Byte	|Нумератор фіскального реєстратора (див. [Таблицю 11](#table11))|
...
|PaymentMethod (завжди =0 для товарних документів)| + |Byte|Форма оплати. 0 - готівкова, 1 - картка через POS-термінал на касі, 2 - кредит, 3 - оплата подарунковим сертифікатом, 4 - оплата бонусами в контексті програми лояльності або бонусна знижка в контексті фіскального обліку (не використовується на касових робочих місцях, але використовується в TCU та Shopserver), 5 - банковський переказ (з рахунку на рахунок), 6 - оплата карткою через інтернет (онлайн). ([public enum PaymentMethod](/Documents/Document.cs))  **Увага! Внаслідок того, що чек може містити декілька ПКО/ВКО з різними формами оплати, коректне значення PaymentMethod має тільки касовий документ, для товарного документа PaymentMethod завжди =0**|
|PromoCodes    	                    | - |Колекція String	|Перелік промокодів, запропонованих покупцем у цьому чеку. Якщо промокод було застосовано, то в MarketingActionRecords буде відповідний запис про спрацювання маркетингового інструменту, пов'язаного з цим промокодом. **Увага!Тут в переліку пишемо всі промокоди, які були запропоновані покупцем, незалежно від того, чи спрацював якийсь з них чи ні.**|
|RoundingSum                        | - |Double	|Сума заокруглення, додана або віднята до суми товарів під час оплати. Використовується в оплатах готівкою, коли потрібно заокруглити суму до порогу заокруглення. У випадку додавання суми заокруглення до суми товарів, значення RoundingSum буде додатнім, у випадку віднімання – від’ємним. У випадку, коли заокруглення не застосовується, RoundingSum = 0. Фактично, ця сума по знаку і значенню є результатом (AmountPaid - Amount). Сума присутня тільки у товарному документі, для касового документу значення завжди дорівнює 0.00|
|SaleChannel		                | + |Int32	|Канал продажу (1 - каса, 2 - E-Commerce, 4 - QR Меню, 8 - Vending Machine, 16 - E-Commerce Mobile тощо)|
...
|Discount                           | + |Double     |Знижка як різниця між реєстровою ціною та факт. роздрібною у поточному документі. тобто, *Discount = PrimaryPrice - SalePrice*|
...
|Quantity                           | + |Double     |Кількість в цьому товарному записі (для документів крім "різницевий перерахунок залишків"). **Увага! Якщо це товар-додаток до іншого товарного запису, то тут потрібно вказати загальну кількість цього товару-додатку. Якщо товар-додаток вилучається, то потрібно вказати кількість 0. Також обов'язково не забувайте для товарів-додатків вказувати у TopDocumentDetailGuid посилання на DocumentDetailGuid товарного запису, якого стосується цей товар-додаток.**|
...
|TopDocumentDetailGuid              | - |Guid       |Guid товарного запису, на який посилається цей товарний запис. Наприклад, товар-доповнення до певної страви. **Для товарів-вилучень в полі Quantity кількість треба вказувати 0**|
...
## <a id="table11">Таблиця 11. Нумератор типів РРО/ПРРО

|Document.FiscalRegisterId (Byte)|
|:---                   |
|0 - РРО/ПРРО не використовується|
|1 - РРО Міні-ФП|
|2 - РРО Datecs 3530 (застарів, не підтримується в нових версіях)|
|3 - РРО Міні-ФП 6|
|4 - РРО Міні-ФП 54|
|~~5 - РРО Штрих-М~~ (підтримка припинена)|
|6 - РРО Міні-ФП 81|
|7 - РРО MG-P800TL/MG-T808TL (застарів, не підтримується в нових версіях)|
|8 - ПРРО Checkbox через Resonance OLE Manager|
|9 - РРО Datecs FP-3141T, FP-T260, FP-320, CMP-10 (протокол OPOS) (застарів, не підтримується в нових версіях)|
|10 - РРО MG-N707TS (застарів, не підтримується в нових версіях)|
|11 - РРО Exellio FPU-550/FPU-260 (протокол Datecs)|
|12 - РРО ICS-810 (прошивка EP-12)|
|~~13 - РРО Atol до версії 10~~ (підтримка припинена)|
|14 - РРО MG-N707TS (OLE Server) Ver.2 (з прошивкою нумерації типів оплат: Готівка=1, Чек=2, Кредит=3, Картка=4) (застарів, не підтримується в нових версіях)|
|~~15 - РРО Atol версії 10~~ (підтримка припинена)|
|16 - РРО AtlasFP (Krypton 5.03 та нов. через інтерфейс Atlas)|
|17 - РРО MG (RS232, протокол, UniDriver)|
|18 - РРО MG (HTTP, JSON протокол, UniDriver)|
|19 - РРО LeoCas (DriverEKKA11, UniDriver)|
